...
状态：2026-09-24 待 GLM 只读安全审查的草案，审查通过并修订后才能交给两条实施路由。此片是 WorkBuddy 项目“任务分享”的一个受限增量，不代表分享面板、附件、搜索、实时协作或项目空间 1:1 验收。WorkBuddy 的可见交互是对标目标；以下表结构与权限是熊宝自己的设计。

## 用户行为与授权边界

- 024 的卡片分享继续默认**只显示标题等安全摘要**。升级到 025 后，所有既有卡片授权的正文权限都必须为 false；代码、迁移和 UI 不得根据 `role='reader'` 自动授权正文。
- 任务本人可以对已有有效卡片授权的**同一项目指定成员**，再单独确认授予“只读对话文本”。确认文案说明：会看到双方已存储的文本消息，包括授权之前的历史和此后新增的文本；文本本身可能包含用户粘贴的路径、链接或敏感内容。本人可分别撤销正文权限或整个卡片授权；撤销卡片须在同一事务内删除正文授权。卡片重新授予后，正文仍须再次确认。
...
正文响应仅允许 `status=ready|pending`、`items:[{seq,role,text,created_at,truncated}]`、`has_more`、`next_before_seq`。`role=user|assistant`，`truncated` 为布尔值。当既有 `thread_history_projection` 尚未 ready 时返回 `status=pending,items=[]`，不触发回填或模型执行；若投影 ready，使用现成 `thread_messages` 行，不访问 checkpoint blob。读取授权与消息行须在一个 DB 事务/一致快照内，PostgreSQL 读时锁当前成员与卡片/正文授权行以与并发撤权串行；SQLite 使用既有事务锁。撤权提交后开始的新请求须 404；已经开始且获授权的响应可能完成，UI 收到后续 404 必须清空缓存。分页游标使用行 `seq`，不依赖 offset；序号既有单调唯一约束使追加消息不会重排旧页。不可向未授权请求暴露 projection 状态或消息数量。

服务端投影必须只解析 `message_json` 的外层字典与 `data.content`，同时核对存储 `role` 和 wire `type` 是匹配的 human/user 或 ai/assistant。只接受纯字符串或明确 `type=text` 的文本块；其它块、工具消息、系统消息、工具调用、思考/推理字段、附加元数据、模型参数、路径/附件结构、异常堆栈全部排除，不把原始 `message_json` 返回客户端。带工具调用的 assistant 行整体不输出；空/无效正文行跳过。分页 `next_before_seq` 以本次扫描的最后一个原始 seq 为准，跳过的行也占游标，避免重复或漏扫。文本中的普通字符原样显示，但前端只以 escaped plain text 渲染，绝不解析 Markdown/HTML 或自动展开 URL。每条输出文本至多 32768 UTF-8 字节，在字符边界截断并设置 `truncated=true`；100 条的页上限因此约为 3.2 MiB 文本。原始 `message_json` 超过 256 KiB 的行整体跳过，避免一次读取被大体积多媒体块拖垮；这类遗漏须在界面如实提示“部分非文本或超大消息未显示”，不能称为完整历史。
...
- 文本投影以真实 `message_to_dict` 形状测试：普通 human/ai、文本块、工具/系统/带 tool_calls 的 ai、思考字段、图片/音视频/文件块、畸形 JSON、超长文本、分页跨被跳过行、未 ready 状态、撤权并发。断言响应不存在 `message_json`、`additional_kwargs`、`tool_calls`、`usage_metadata`、`session_key`、`artifact`、`path` 等结构字段。
