...
| PS-03A | 项目动态读模型：成员、邀请、待办安全事件分页，留言创建/读取；“与我相关/成员动态”筛选 | 018–020；必须遵守 021 私密事件禁写合同 | 两成员与外人验证过滤/顺序/刷新；任何事件均无私密任务 ID、凭据或项目指令正文；留言撤权后不可读。 |
| PS-06A | 项目资产根：文件夹、上传、列表/搜索/类型/容量、版本/下载与删除 | 019 成员 ACL，独立存储与垃圾回收设计 | 猜测文件名/版本 ID 无法越权下载；上传失败不留下可见半成品；版本可恢复且容量与实际存储一致。 |
