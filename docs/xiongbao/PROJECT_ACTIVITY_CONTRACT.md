...
状态：2026-09-24 的 022 第一片由 `aeb8e01d` 实现，GLM 对该固定提交完成只读审查（无 P0/P1），其可修 P2 由 Codex 在 `6e4a216e` 修复。最终受影响测试、类型检查、构建及隔离三身份浏览器关键链路通过；PostgreSQL 实库、完整 WorkBuddy 视觉/交互对照仍待验。依赖 018–021，022 提供 SQLite 和 PostgreSQL 配对脚本。本片把现有项目安全事件与新建的纯文本留言展示为项目动态，支持“与我相关 / 成员动态”及稳定分页；不把任务私聊、邀请密钥、项目指令或连接器凭据发布到时间线。WorkBuddy 5.5.6 的动态页签和筛选已在本机只读观察，留言能力另见官方更新日志；以下接口与权限是熊宝自己的设计。
